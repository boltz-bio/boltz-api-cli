...
var smallMoleculeLibraryScreenEstimateCost = requestflag.WithInnerFlags(cli.Command{
	...
	"target": {
		...
		&requestflag.InnerFlag[[]map[string]any]{
			Name:       "target.bonds",
			Usage:      "Covalent bond constraints between atoms in the target complex. Atom-level ligand references support CCD atom names and explicitly atom-mapped SMILES atoms; for SMILES ligands, use numeric atom-map notation (for example, [C:1] is referenced as C1).",
			InnerField: "bonds",
		},
		&requestflag.InnerFlag[[]map[string]any]{
...
var smallMoleculeLibraryScreenResume = cli.Command{
	Name:    "resume",
	Usage:   "Resume a stopped small molecule library screen from its last checkpoint",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "id",
	...
	Action:          handleSmallMoleculeLibraryScreenResume,
	HideHelpCommand: true,
}
...
func handleSmallMoleculeLibraryScreenResume(ctx context.Context, cmd *cli.Command) error {
	client := boltzapi.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("id") && len(unusedArgs) > 0 {
		cmd.Set("id", unusedArgs[0])
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "small-molecule:library-screen resume",
		Transform:      transform,
	})
}
